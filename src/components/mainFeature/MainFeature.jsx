    ...
    return (
        <div className="my-10 mx-5 md:mx-16 lg:mx-72">
            <div className="flex flex-col gap-y-4 md:flex-row md:justify-between items-center">
                <p className="font-bold text-lg md:text-2xl">{currentButton ? `Available Players` : `Selected Players (${khelowar.length}/6)`}</p>
                <div>
                    ...
                    className={`md:col-span-1 px-4 py-3 md:px-6 md:py-2 border border-gray-200 rounded-r-xl font-semibold ${currentButton ? 'bg-white hover:bg-gray-100 transition ease-in-out' : 'bg-buty'}`}>Selected ({khelowar.length})</button>
                </div>
            </div>
            <div className="my-10">
